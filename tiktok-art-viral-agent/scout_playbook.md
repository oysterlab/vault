...
- 첫 번째 행 y ≈ 450 (탭 바 아래)
- ⚠️ **R28 수정**: 그리드 아이템 실제 좌표는 UI dump FrameLayout bounds로 확인해야 정확. 좌열 이미지 중심 **(231, 512)**, 우열 **(672, 512)**. y=450은 탭 바 / 카드 타이틀 영역과 겹칠 수 있음
- 탭 후 로딩 대기: **2.5초** 필요
- ⚠️ **R28 수정**: 포스트 열림 확인은 `qvk` (likes 텍스트) 또는 `qwb` (캡션) resource_id 존재 여부로 판단. 이전 `qmd` resource_id는 현재 TikTok 버전에서 없음
- **engagement 데이터 수집 resource_id (R28 확인)**:
  - 계정명: `j5j` (text = account name, @ 없이)
  - 캡션: `qwb` (text = caption)
  - 좋아요: `qvk` (text = "3,174" 또는 "3.7만")
  - 댓글: `qvg` (text = count)
  - 공유: `qvp` (text = count)
  - 저장: `qvs` (text = count)
  - 팔로우 버튼: `sfb` (text = "팔로우")
- ⚠️ 이전 `content_desc`에 "좋아요" 패턴은 더 이상 동작하지 않음
- ⚠️ **R28 발견**: FYP에서 검색 진입 시 검색 아이콘(돋보기) 좌표는 **(880, 89)** — 상단 우측. 하단 "검색" 바(404, 2015)는 현재 버전에서 없을 수 있음
- ⚠️ **R28 발견**: TikTok 시작 시 "키보드 동작" 오버레이가 뜰 수 있음 (resource_id `wea`). "확인" 버튼(452, 1868) 탭으로 닫기. 단, UI dump에서 계속 감지될 수 있으나 실제 화면에는 안 보임 — 무시해도 됨
...
| Turner painting | R20 | 0 | 4 | 0% | — | ❌ 10라운드 간격에도 전부 R10 중복. 결과 고정. failed 추가. |
| Botticelli painting | R20 | 1 | 4 | 25% | 100 (Erosý.Art) | R12 이후 8라운드 간격. 중복 3개. 결과 고정 조짐. |
| Hokusai wave | R20 | 3 | 4 | 75% | 101 (rockrapdesigns) | R12 이후 8라운드 간격. 중복 1개. 양호. |
| Cezanne painting | R20 | 4 | 4 | 100% | 973 (BrushAndTaless) | ★100% 히트! R12 이후 8라운드 간격. 안정적. |
| Waterhouse painting | R20 | 2 | 4 | 50% | 1,648 (Laura's favorite things) | R13 이후 7라운드 간격. 중복 2개. 1,648 likes Top 포스트(Share 21%). |
| Hiroshige ukiyo-e | R20 | 1 | 4 | 25% | 35 (absurdical) | R13 이후 7라운드 간격. 중복 3개. 결과 고정. |
| Renaissance Madonna | R20 | 4 | 4 | 100% | 593 (History) | ★100% 히트! R13 이후 7라운드 간격. Pietà 593 likes. |
| Goya painting | R22 | 2 | 4 | 50% | 53.2만 (PhilosoForum) | R12 이후 10라운드 간격. 중복 2개. 53.2만 대형 포스트! |
| pre raphaelite art | R22 | 3 | 4 | 75% | 8,673 (BrushAndTaless) | R12 이후 10라운드 간격. 중복 1개. 안정적. |
| Dutch Golden Age art | R22 | 3 | 4 | 75% | 335 (THE ART) | R13 이후 9라운드 간격. 중복 1개. 양호. |
| famous sculpture history | R22 | 3 | 4 | 75% | 185 (the dictionary of culture) | R6 이후 16라운드 간격. 중복 1개. |
| Morisot painting | R22 | 4 | 4 | 100% | 1,684 (the_inspire_gallery) | ★100% 히트! R16 이후 6라운드 간격. 안정적. |
| Lapis lazuli art history | R22 | 4 | 4 | 100% | 1.7만 (Julia) | ★100% 히트! R19 이후 3라운드 간격에도 100%. 결과 유연성 최고. |
| Rodin sculpture | R22 | 4 | 4 | 100% | 515 ($) | ★100% 히트! R17 이후 5라운드 간격. 안정적. |
...
| keyword_search → 사진 tab (7 keywords + Row 2 스크롤) | R20 | 15 unique / 4.7분 | 7/7 키워드 사진탭 성공. "Cezanne" 4/4 100%, "Renaissance Madonna" 4/4 100%. "Turner" 0개(전부 중복, failed 추가). 1,648 likes Top 1(Share 21%). |
| keyword_search → 사진 tab (7 keywords + Row 2 스크롤) | R22 | 23 unique / 4.2분 | 7/7 키워드 사진탭 성공. "Morisot"/"Lapis lazuli"/"Rodin" 3개 100%. UI 변경 대응 (rid→position 기반 수집). 53.2만 likes 초대형 발견. |

### 효율 높은 키워드 후보 (R23에서 시도)
- "women in art history" → R21 사용. 2라운드 간격. R24+.
- "Raphael school athens" → R21 사용. 2라운드 간격. R24+.
- "ancient Egyptian art" → R21 사용. 2라운드 간격. R24+.
- "Bernini sculpture" → R14 이후 8라운드 간격. 적합.
- "Fayum portrait" → R19 이후 4라운드 간격. 최소 5필요이므로 R24+.
- "Van Eyck painting" → R18 이후 5라운드 간격. 적합.
- "Rubens painting" → R18 이후 5라운드 간격. 적합.
- "El Greco art" → R18 이후 5라운드 간격. 적합.
- "Michelangelo art" → R13 이후 10라운드 간격. 적합.
- "Chinese landscape painting" → R16 이후 7라운드 간격. 적합.
- **⚠️ R22 교훈**: TikTok UI 변경 — rid 기반 engagement 수집 불가. position-based 수집 필요. 그리드 Row 1 y=480으로 보정.
- **⚠️ R22 교훈**: "Top N 카운트다운 + 투표 CTA" = 메가히트 포맷 (PhilosoForum 53.2만).
- **⚠️ R22 교훈**: 안료/재료 기반 횡단 서사 = 높은 Share rate 13.5% (Julia 1.7만).
- **⚠️ R22 교훈**: 콜라주 포맷 4분할 = 적은 슬라이드에 높은 정보 밀도.
...
### R23 (2026-04-08)
- **Duration**: 283초 | **Found**: 15 unique | **CAPTCHA**: No
- **Keywords**: Michelangelo art (2/4), Monet water lilies (4/4 ★), Caravaggio painting (0/0 fail), El Greco art (2/4), Da Vinci secrets (3/4), Fayum portrait (0/0 fail), art heist history (4/4 ★)
- **핵심 교훈**:
  1. **2개 키워드 사진탭 미발견** (Caravaggio painting, Fayum portrait) — Caravaggio는 R6 100% → R23 fail. Fayum은 R16 100% → R19 0(중복) → R23 fail. 두 키워드 모두 failed_keywords 후보.
  2. **"Da Vinci secrets" Top 1+Top 2 동시 차지** — 22만 likes (@Note from Jesus, Last Supper hidden music, "Normal painting?" 2단어 훅) + 1.9만 likes (@CultureDetective, Mona Lisa plumbonacrite, 15슬라이드 화학 분석). **"숨겨진 비밀+유명 작품" 포맷이 두 라운드 연속 메가히트**.
  3. **15슬라이드 long-form 텍스트 에세이형** — @CultureDetective의 길이가 1.9만 likes에 기여한 핵심 가설. completion 알고리즘 보상.
  4. **@emalamenart Save rate 12%** — Like 5,531 / Save 662. 미술관 공간 투어 포맷 = 여행 청중 흡수.
  5. **art heist history 4/4 부활** — R8 100% 후 처음 재시도. 안정 키워드 등극.
  6. **Phase 2 reopen 신뢰성 문제** — Top 1 (Note from Jesus) reopen 시 동일 키워드/유사 캡션의 다른 포스트(@welcome 4,819)에 매칭됨. 캡션 30자 snippet은 너무 약한 식별자. **개선안**: 계정명 + 캡션 첫 50자 동시 검증.

### R26 (2026-04-10)
- **Duration**: 294초 | **Found**: 27 unique | **Filter**: painting-only, single-artwork only
- **Keywords**: Hieronymus Bosch (5/5), Goya painting (6/5+), pre raphaelite art (6/5+), Dutch Golden Age painting (5/5), Hokusai wave (5/5), Waterhouse (0 dup), painting hidden details (time out)
- **Top 3 (single-painting only)**:
  1. @Footnotes in Time — 60,000 likes — "The Black Brunswicker" (Millais) — 7 slides, share rate 8.1%
  2. @historybyart — 4,965 likes — "The Mona Lisa of the North?" (Pearl Earring) — 8 slides, share 9.8%
  3. @historybyart — 3,205 likes — "The Wave is Alive" (Hokusai Great Wave) — 10 slides, share 17.6%
- **핵심 교훈**:
  1. **PhilosoForum 532K + 130K 둘 다 "Top 8 Paintings of [X]" 리스티클** — 우리 파이프라인 비카피. R22 53.2만 패턴과 동일 — PhilosoForum은 단일작품 deep-dive 안 함.
  2. **filter 제외 케이스 학습**: 책 다이어그램, 건축가(Zaha Hadid), 현대 초현실화가(Jacek Yerka), 상징 편람(7개 다른 그림 사용), 작가 전기(pocket.art.gallery) — caption만으로 판별 어려움, **스크린샷 직접 확인 필수**.
  3. **@historybyart 2개 입성** — 우리 파이프라인과 가장 유사한 포맷. R27+ 프로필 직접 탐색 전략 가치 높음.
  4. **Share rate 패턴**: 세 Top 모두 share > save. Trivia + aphorism 조합이 Share 드라이버. R26 Top 3 avg share rate 11.8%.
  5. **Position-based extraction 재확인** — resource_id 필드가 빈 값. y=2080-2220 engagement bar, y<230 account, y=1400-2000 caption.
  6. **그리드 좌표 업데이트**: Row 1 center y=600 (기존 playbook의 y=480은 오래된 값), Row 2 y=1500 (스크롤 없이 접근 가능), Row 3은 swipe 후 y=1500.
| painting hidden details | R27 | 4 | 4 | 100% | 14.7만 (historybyart) | ★★ 100% 히트! R4 이후 23라운드 간격. 14.7만 대형 포스트. Arnolfini Portrait. proven 추가. |
| Vermeer painting | R27 | 0 | 0 | 0% | — | ❌ 사진탭 미발견. R13 이후 14라운드 간격에도 실패. failed 추가. |
| Caravaggio painting | R27 | 3 | 4 | 75% | 5,648 (chrysa.) | R11 이후 16라운드 간격. chrysa 5.6K, 카라바조 손 디테일. |
| art mystery solved | R27 | 4 | 4 | 100% | 21 (향이) | ★100% 히트이나 전부 한국어 콘텐츠 (21/1/9/4 likes). 영어 고전미술 소진. |
| Botticelli painting | R27 | 0 | 0 | 0% | — | ❌ 사진탭 미발견. R20 이후 7라운드 간격에도 실패. failed 추가. |
| women in art history | R27 | 2 | 4 | 50% | 9.7만 (History👑) | R21 이후 6라운드 간격. 중복 2개. Margaret Beaufort 초상화 9.7만. |
| Lapis lazuli art history | R27 | 3 | 4 | 75% | 5.1만 (Historygeek) | R22 이후 5라운드 간격. 5.1만이나 비미술(countries). historybyart 615 (Holbein Ambassadors). |
